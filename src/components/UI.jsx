...
export const UI = ({ hidden, ...props }) => {
  const input = useRef();
  const { chat, loading, cameraZoomed, setCameraZoomed, response } = useChat();

  const sendMessage = () => {
    const text = input.current.value;
    if (!loading && text) {
      chat(text); // Send user message to chat function
      input.current.value = "";
    }
  };
      ...
      <div className="fixed top-0 left-0 right-0 bottom-0 z-10 flex justify-between p-4 flex-col pointer-events-none">
        ...
        <div className="mt-4 max-w-screen-sm w-full mx-auto">
          ...
          <div className="w-full placeholder:text-gray-800 placeholder:italic p-4 rounded-md bg-opacity-50 bg-white backdrop-blur-md">
            {loading ? (
              <p className="text-gray-500 italic">Generating response...</p>
            ) : (
